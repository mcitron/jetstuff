...
//   TFile * f = TFile::Open("./batch/qcd_test/qcd_calib.root");
 ...
 TFile * f = TFile::Open("./global_PUS_TEST.root");
   ...
   PUSregime.push_back("5440_nopus");
   PUSregime.push_back("5430_nopus");
   PUSregime.push_back("5420_nopus");
  ...
  // PUSregime.push_back("4300_donut");
   ...
   {      
      ...
      {
	 TH1D * denom = f->Get(("demo/"+*iPUS+"_gen/other/col2_"+*iJet+"_pt").Data());
	 denom->Rebin(10);
	 denom->Sumw2();
	 for (auto iAlgo = algo.begin(); iAlgo!=algo.end(); iAlgo++)
	 {
	    TH1D *num=(f->Get(("demo/"+*iPUS+"_gen/other/col2_matched_"+*iAlgo+"_"+*iJet+"_pt").Data()));
	    num->Rebin(10);
	    num->Sumw2();

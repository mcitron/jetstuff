...
void L1CaloTowerProducer::produce( edm::Event & aEvent, const edm::EventSetup & aSetup )
{
	...
	for ( EcalTrigPrimDigiCollection::const_iterator lEcalTPItr = lEcalDigiHandle->begin(  ); lEcalTPItr != lEcalDigiHandle->end(  ); ++lEcalTPItr ) {
	  ...
	  //std::cout << lEcalTPItr->compressedEt() << ", " << lEcalTPItr->id().ieta() << ", " <<  lEcalTPItr->id().iphi() << std::endl;
	}
	...
	if ( !mUseupgradehcal )
	{
		...
		for ( HcalTrigPrimDigiCollection::const_iterator lHcalTPItr = lHcalDigiHandle->begin(  ); lHcalTPItr != lHcalDigiHandle->end(  ); ++lHcalTPItr ) {
		  ...
		  //std::cout << lHcalTPItr->SOI_compressedEt() << ", " << lHcalTPItr->id().ieta() << ", " <<  lHcalTPItr->id().iphi() << std::endl;
		}
	}
	else
	{
